...
async function init() {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS pinned_messages (id SERIAL PRIMARY KEY, message_id INTEGER, chat_id INTEGER, pinned_by INTEGER, pinned_at TIMESTAMP DEFAULT NOW())`);
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS user_settings (user_id INTEGER, setting_key VARCHAR(100), setting_value TEXT, PRIMARY KEY(user_id, setting_key))`);
    await pool.query(`CREATE TABLE IF NOT EXISTS schedule_groups (id SERIAL PRIMARY KEY, name VARCHAR(100), color VARCHAR(7))`);
    await pool.query(`CREATE TABLE IF NOT EXISTS schedule_lessons (id SERIAL PRIMARY KEY, group_id INTEGER, day_of_week INTEGER, start_time TIME, end_time TIME, title VARCHAR(255), description TEXT, is_common BOOLEAN, status VARCHAR(20) DEFAULT 'active', event_type VARCHAR(20), lesson_date DATE, created_at TIMESTAMP DEFAULT NOW())`);
    
    // Новая таблица хранилища с BYTEA
...
app.post('/api/send-verification', async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO email_verifications (email, code, expires_at) VALUES ($1,$2,$3)', [req.body.email, code, new Date(Date.now() + 10*60*1000)]);
        await resend.emails.send({ from: process.env.RESEND_FROM_EMAIL || 'onboarding@resend.dev', to: req.body.email, subject: 'Код подтверждения', html: `<h2>${code}</h2>` });
        res.json({ success: true });
    } catch (e) { res.json({ success: false }); }
});
...
app.post('/api/verify-email', async (req, res) => {
    try {
        const r = await pool.query('SELECT * FROM email_verifications WHERE email=$1 AND code=$2 AND expires_at > NOW()', [req.body.email, req.body.code]);
        if (r.rows.length === 0) return res.json({ success: false });
        await pool.query('UPDATE users SET email_verified=true WHERE email=$1', [req.body.email]);
        res.json({ success: true });
    } catch (e) { res.json({ success: false }); }
});
...
app.get('/api/users', async (req, res) => {
    try { const r = await pool.query('SELECT id, name, email, role, username FROM users ORDER BY name'); res.json({ success: true, users: r.rows }); } catch (e) { res.json({ success: false }); }
});
...
app.get('/api/schedule/groups', async (req, res) => {
    try { const r = await pool.query('SELECT * FROM schedule_groups ORDER BY id'); res.json({ success: true, groups: r.rows }); } catch (e) { res.json({ success: false }); }
});
...
app.get('/api/storage/file/:id/:type', async (req, res) => {
    ...
    try {
        ...
        if (r.rows.length === 0 || !r.rows[0].data) return res.status(404).send('Нет файла');
...
app.delete('/api/admin/storage/:id', async (req, res) => {
    try { await pool.query('DELETE FROM storage_items WHERE id=$1', [req.params.id]); res.json({ success: true }); } catch (e) { res.json({ success: false }); }
});
...
io.on('connection', async (socket) => {
    ...
    const history = chatId 
        ? (await pool.query('SELECT id, user_name as name, text, user_id, image_url, timestamp, edited FROM messages WHERE chat_id=$1 ORDER BY timestamp ASC LIMIT 100', [chatId])).rows
        : (await pool.query('SELECT id, user_name as name, text, user_id, image_url, timestamp, edited FROM messages WHERE chat_id IS NULL ORDER BY timestamp ASC LIMIT 100')).rows;
