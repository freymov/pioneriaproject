...
app.get('/api/storage', async (req, res) => {
    try {
        const result = await pool.query('SELECT id, title, pdf_name, mp3_name, mp4_name, created_at FROM storage_items ORDER BY created_at DESC');
...
app.get('/api/storage/file/:id/:type', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`SELECT ${col} as data, ${nameCol} as name FROM storage_items WHERE id = $1`, [id]);
        if (result.rows.length === 0 || !result.rows[0].data) return res.status(404).send('Файл не найден');
        
        const mime = { pdf: 'application/pdf', mp3: 'audio/mpeg', mp4: 'video/mp4' };
        res.setHeader('Content-Type', mime[type]);
        res.setHeader('Content-Disposition', `inline; filename="${result.rows[0].name}"`);
...
app.post('/api/admin/storage', storageUpload.fields([
    ...
    try {
        ...
        if (!pdf && !mp3 && !mp4) return res.json({ success: false, error: 'Загрузите хотя бы один файл' });
